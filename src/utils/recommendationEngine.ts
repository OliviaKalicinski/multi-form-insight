...
export const gerarRecomendacoes = (
  ...
  // REGRA: Taxa Recompra é DIAGNOSTIC → NÃO pode gerar recomendações
  // Mas podemos usar Churn (DECISIONAL) como base para retenção
  ...
  // REGRA: Ticket Médio é DIAGNOSTIC → NÃO pode gerar recomendações
  // Esta recomendação é BLOQUEADA porque ticketMedio não é DECISIONAL
  // NOTA: Mantida comentada para documentar a governança
  /*
  if (benchmarks.ticketMedio && atual.vendas.ticketMedioReal < benchmarks.ticketMedio) {
    // BLOQUEADO: ticketMedio é DIAGNOSTIC, não pode gerar recomendação
    // canGenerateRecommendation(authority.ticketMedioReal) === false
  }
  */
  
  // Rec 4: Otimizar Operações (threshold fixo, mas tempoEmissaoNF é DIAGNOSTIC)
  // NOTA: Operações não tem métrica DECISIONAL, então recomendações operacionais são informativas
  // Mantida porque é baseada em threshold fixo (não benchmark), e é uma sugestão operacional
  if (atual.operacoes.tempoEmissaoNF > 3.0) {
    recomendacoes.push({
      ...
      prioridade: 0,
      facilidade: 'baixa',
      basedOnMetric: 'tempoEmissaoNF (threshold fixo)',
    });
  }
