...
import {
  AlertTriangle, MessageCircle, CheckCircle, Clock, Send,
  EyeOff, RefreshCw, Filter, TrendingDown, Heart, HelpCircle, Shield,
} from "lucide-react";
...
// ─── Helpers ────────────────────────────────────────────────────────────────
const RISCO_CONFIG = {
  critico: { label: "Crítico", bg: "bg-red-100 text-red-800 border-red-300",    dot: "bg-red-500",    icon: <AlertTriangle className="h-3 w-3" /> },
  alto:    { label: "Alto",    bg: "bg-orange-100 text-orange-800 border-orange-300", dot: "bg-orange-500", icon: <AlertTriangle className="h-3 w-3" /> },
  medio:   { label: "Médio",   bg: "bg-amber-100 text-amber-800 border-amber-300",    dot: "bg-amber-400",  icon: <Clock className="h-3 w-3" /> },
  baixo:   { label: "Baixo",   bg: "bg-green-100 text-green-800 border-green-300",    dot: "bg-green-500",  icon: <CheckCircle className="h-3 w-3" /> },
};
...
export default function ComentariosInstagram() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      toast({ title: `Sync concluído`, description: `${data.comments} comentários encontrados, ${data.classified} classificados.` });
      await fetchComments();
    } catch (e: any) {
  ...
  const handleHide = async (id: string) => {
    await supabase.from("instagram_comments").update({ oculto: true }).eq("id", id);
  ...
  const kpis = useMemo(() => ({
    ...
    taxaResposta:   comments.length > 0
  ...
  return (
    <div className="container mx-auto px-6 py-8 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: "Não respondidos", value: kpis.naoRespondidos, color: kpis.naoRespondidos > 0 ? "text-amber-600" : "" },
          { label: "Críticos / Altos", value: `${kpis.criticos} / ${kpis.altos}`, color: kpis.criticos > 0 ? "text-red-600" : "" },
          { label: "Taxa de resposta", value: `${kpis.taxaResposta}%`, color: kpis.taxaResposta >= 80 ? "text-green-700" : "text-amber-600" },
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map(comment => {
            ...
            return (
              <Card key={comment.id} className={`border ${comment.risco === "critico" ? "border-red-300 bg-red-50/30" : comment.risco === "alto" ? "border-orange-200" : ""}`}>
                <CardContent className="pt-4 pb-3 px-4">
                  ...
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-2 flex-wrap">
                      {/* Risco badge */}
                      <span className={`flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full border ${rc.bg}`}>
                        {rc.icon} {rc.label}
                      </span>
                      {/* Sentimento */}
                      {comment.sentimento && (
                  ...
                  {isReplying && (
                    <div className="mt-3 space-y-2">
                      ...
                      <div className="flex gap-2 justify-end">
                        ...
                        <Button size="sm" onClick={() => handleReply(comment)} disabled={sendingReply || !replyText.trim()} className="gap-1">
