...
import { filterAdsByMonth, calculateAdsMetrics } from "@/utils/adsCalculator";
...
const Ads = () => {
  ...
  const handleAdsDataLoaded = (data: AdsData[]) => {
    ...
    if (data.length > 0) {
      try {
        const firstDate = new Date(data[0]["Início dos relatórios"]);
        setSelectedMonth(format(firstDate, "yyyy-MM"));
      } catch (error) {
        console.error("Error setting initial month:", error);
      }
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <h1 className="text-4xl font-bold text-foreground">💰 Anúncios - Meta Ads</h1>
            <p className="text-muted-foreground">
              Análise completa de performance dos anúncios do Facebook e Instagram
            </p>
          </div>
        </div>
          ...
          <Card>
            <CardContent className="py-12 text-center">
              <p className="text-muted-foreground">
                ...
                Faça upload de um arquivo CSV/TSV do Meta Ads Manager.
              </p>
            </CardContent>
          </Card>
        ) : (
