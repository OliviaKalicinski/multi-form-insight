...
export default function Publico() {
  const { audienceData } = useDashboard();
  ...
  if (!audienceData || !metrics) {
    return (
      <div className="container mx-auto px-6 py-8">
        <EmptyState
          icon={<Users className="h-8 w-8 text-muted-foreground" />}
          title="Nenhum dado demográfico encontrado"
          description="Faça upload do arquivo CSV de público do Instagram na página de Upload para visualizar as análises."
  ...
  const citiesChartData = audienceData.cidades.slice(0, 10).map((city, index) => ({
    cidade: city.cidade.split(",")[0], // Just city name, no state
    ...
    <div className="container mx-auto px-6 py-8 space-y-8">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Perfil demográfico dos seguidores no Instagram
        </p>
      </div>
