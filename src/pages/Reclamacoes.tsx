...
export default function Reclamacoes() {
    ...
    if (search) {
      ...
      list = list.filter((c) => {
        ...
        return (
          ...
          (c.tipo_reclamacao ?? "").toLowerCase().includes(q)
        );
      });
    }
  ...
  const exportCSV = () => {
    ...
    const rows = filtered.map((c) => [
      ...
      q(customerMap.get(c.customer_id) ?? ""),
      q(c.tipo_reclamacao ?? ""),
      c.gravidade ?? "",
      c.status,
      q(c.atendente ?? ""),
            ...
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="cursor-pointer select-none" onClick={() => handleSort("gravidade")}>
                  <span className="flex items-center">
                    Gravidade <SortIcon col="gravidade" />
                  </span>
                </TableHead>
                ...
                <TableHead className="cursor-pointer select-none" onClick={() => handleSort("atendente")}>
                  <span className="flex items-center">
                    Atendente <SortIcon col="atendente" />
                  </span>
                </TableHead>
                <TableHead></TableHead>
              </TableRow>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((c) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      {c.gravidade && (
                        <Badge variant="outline" className={`text-[10px] ${gravidadeColors[c.gravidade] ?? ""}`}>
